...
/* Waits for thread TID to die and returns its exit status.  If
   ...
   does nothing. */
bool check_child(tid_t child_tid)
{
  struct list_elem * itr;
  for(itr = list_begin(&thread_current ()->child_list);itr != list_end(&thread_current ()->child_list);itr = list_next(itr))
  {
    struct thread *tt = list_entry(itr,struct thread, child_elem);
    if(tt->tid == child_tid)
      return true;
  }
  return false;
}
void kill_child(tid_t child_tid)
{
  struct list_elem * itr;
  for(itr = list_begin(&thread_current ()->child_list);itr != list_end(&thread_current ()->child_list);itr = list_next(itr))
  {
...
process_wait (tid_t child_tid)
{
  ...
  if(!check_child(child_tid))
    return -1;
  sema_down(&child->sema1);
  int status_ = child->exit_status;
  kill_child(child_tid); 
  sema_up(&child->sema2);
  return status_;
}
...
process_exit (void)
{
  ...
  {
    itr = list_pop_front(&curr->child_list);
    tt = list_entry(itr, struct thread, child_elem);
    sema_up(&tt->sema2);
  }
